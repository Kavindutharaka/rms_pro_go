...
app.controller('HomeCtrl', function ($scope, $http, $filter, $rootScope, $window, $timeout) {
    ...
    var myModal_ctb = document.getElementById("myModal_ctb");
    var myModal_dend = document.getElementById("myModal_dend");
    ...
    $scope.make_bill = function () {
        p_d = ' <div style="font-size:14px;padding-bottom:5px;border-bottom:solid 2px;margin-top:10px;"> ' +
            ...
            '    <div style="display:inline-block;">STW:' + $scope.tun + '</div> <div style="display:inline-block;float: right;"> TBN: ' + $scope.tbn + '</div>  ' +
            '  </div > ' +
            '  </div > ' +
        ...
        var printData = [
            {
                ...
                data: '<html>' +
                    ...
                    '<div style="font-size:35px;text-align:center;width:99%;">SAMUEL7</div>' +
                    '<div style="font-size:20px;text-align:center;width:99%;">RESTAURANT & BAR</div>' +
   ...
   // modal_b.style.display = 'block';
    ...
    $scope.load_tb_data_sel = function () {
        ...
        $http.post('./api/WorkApi/item_by_tb', JSON.stringify(bbb)).then(function (responsea) {
            ...
            $scope.s_chg = 0;
            $scope.g_tot = 0;
            $scope.tun = $scope.item_by_tb[0].tun;
            $scope.tbn = $scope.item_by_tb[0].tbn;
            for (var c = 0; c < $scope.item_by_tb.length; c++) {
    ...
    $scope.make_discount = function () {
        console.log($scope.g_tot);
        var end_tot = Number($scope.g_tot) - (Number($scope.g_tot) / 100) * Number($scope.bdis);
        $scope.bill_poot.push({ n: 'Bill Discount %', v: $scope.bdis });
        $scope.bill_poot.push({ n: 'End Total ', v: end_tot });
    }; 

    $scope.read_discount_val = function () {
        var diss = $scope.bdis;
        if (Number($scope.bdis) > 100) { $scope.bdis = 0; }
        if (Number($scope.bdis) < 0) { $scope.bdis = 0; }
    };
    $scope.finsh_bill_tb = function () {
      ...
      //  modal.style.display = "block";
        ...
        if (Number($scope.tb_billing) == 1) {
            ...
            $scope.grand_tot = Number($scope.total) + Number($scope.serv_chg);
            var end_tot = Number($scope.bill_amount) - (Number($scope.bill_amount) / 100) * Number($scope.bdis);
            $scope.bill_amount = end_tot;
    ...
    $scope.change_tb_op = function () {
        myModal_ctb.style.display = "block";
        var bbb = {
            "SysID": "SELECT *  FROM [dbo].[tb_list] where u = 0 and s = 1 order by n;"
        };
        $http.post('./api/MasterApi/sp', JSON.stringify(bbb)).then(function (responsea) {
            $scope.free_tbLst = responsea.data;
            console.log(responsea.data);
        }, function (responsea) { });
    };

    $scope.change_tb_opcls = function () {
        myModal_ctb.style.display = "none";
        myModal_dend.style.display = "none";
    };

    $scope.open_dayend = function () {
        myModal_dend.style.display = "block";
    };

    $scope.new_tb_id = 0;
    $scope.chage_tb_do = function () {
        myModal_ctb.style.display = "none";
    //    console.log($scope.seled_tb_id);
    //    console.log($scope.new_tb_id);
        var bbb = {
            "SysID": "exec [dbo].[change_tb] '" + $scope.seled_tb_id + "','" + $scope.new_tb_id +"'"
        };
        $http.post('./api/MasterApi/sp', JSON.stringify(bbb)).then(function (responsea) {
            $window.location.reload();
        }, function (responsea) { });
    ...
    $scope.do_dayendnpow = function () {
        myModal_dend.style.display = "none";
        var bbb = {
            "SysID": "exec [dbo].[DoDayEnd] '" + $scope.uid +"','" + $filter('date')(new Date(), 'yyyy/MM/dd HH:mm:ss') + "'"
        };
